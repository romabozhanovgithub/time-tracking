...
export const calendarReducer = (state = {}, action) => {
    switch (action.type) {
        ...
        case CALENDAR_CREATE_SUCCESS:
            const calendarCreateList = [...state.calendars, action.payload].sort(
                (calendarPrevios, calendarCurrent) => new Date(calendarCurrent.time_start).getTime() - new Date(calendarPrevios.time_start).getTime()
            )
